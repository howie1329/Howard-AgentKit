...
const validInstallModes: InstallMode[] = ["template", "skill"];
...
async function collectInstallableTemplatePaths(dir: string, base: string): Promise<string[]> {
...
function isDesignSystemName(value: string): value is DesignSystemName {
  return validDesignSystems.includes(value as DesignSystemName);
}
...
function formatDesignSystemList(): string {
  return validDesignSystems.join(", ");
}

function resolveDesignSystem(name: string): DesignSystemName {
...
function effectiveDesignSystem(name: string | undefined): DesignSystemName {
...
function readConfigAiTools(value: unknown): AiToolName[] | undefined {
  if (value === undefined) {
    return undefined;
  }
...
function readConfigPersonalization(value: unknown): PersonalizationValues | undefined {
  if (value === undefined) {
    return undefined;
  }
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    personalization[key as keyof PersonalizationValues] = entry;
  }

  return personalization;
}

function resolveInstallMode(installMode: string): InstallMode {
  const normalizedInstallMode = installMode.toLowerCase();

  if (!validInstallModes.includes(normalizedInstallMode as InstallMode)) {
    throw new Error(
      `Unknown install mode "${installMode}". Valid install modes: ${validInstallModes.join(", ")}.`,
    );
  }

  return normalizedInstallMode as InstallMode;
}

export function getInstallMode(config?: AgentKitConfig): InstallMode {
...
async function promptForTextValue(
  message: string,
  placeholder: string,
...
async function promptForPersonalization(defaults: PersonalizationValues | undefined): Promise<PersonalizationValues | undefined> {
  const shouldPersonalize = resolvePrompt(
    await confirm({
  ...
  return {
    ...
    projectDescription: await promptForTextValue(
    ...
    designSystemPath: await promptForTextValue(
      "Design system path",
      "docs/design-system.md",
      defaults?.designSystemPath,
    ),
    ...
    stackSummary: await promptForTextValue(
      "Stack summary",
      "Next.js, TypeScript, Tailwind CSS, Vitest",
      defaults?.stackSummary,
    ),
  };
}
...
async function installFileIfAllowed(
  targetDir: string,
  file: string,
  options: Pick<InitOptions, "dryRun" | "force">,
  result: Pick<InstallResult, "created" | "skipped">,
...
async function resolveInitTemplateFiles(options: InitOptions): Promise<string[]> {
...
async function installTemplates(
  ...
  const result: InstallResult = { targetDir, created: [], skipped: [] };
  ...
  if (options.writeConfig) {
    await installFileIfAllowed(targetDir, configFileName, options, result, () =>
      serializeConfig(getResolvedConfig(options, "template", agentkitVersion)),
    );
  }
  ...
  if (preset) {
    await installFileIfAllowed(targetDir, "STACK.md", options, result, () =>
      wrapManagedBlock("STACK.md", getStackGuidance(preset)),
    );
  }
...
async function buildTemplateContent(
  file: string,
  preset: PresetName | undefined,
  designSystem: DesignSystemName,
): Promise<string> {
...
async function promptForTarget(target: string | undefined): Promise<string> {
  if (target && target !== ".") {
    return target;
  }
...
async function promptForProjectPreset(): Promise<PresetName | undefined> {
  const projectType = resolvePrompt(
    await select<ProjectTypeName>({
...
async function promptForAiTools(defaults: AiToolName[] | undefined): Promise<AiToolName[]> {
  return resolvePrompt(
    await multiselect<AiToolName>({
...
async function promptForTemplateSet(defaultValue: TemplateSetName | undefined): Promise<TemplateSetName> {
  return resolvePrompt(
    await select<TemplateSetName>({
...
async function promptForDesignSystem(defaultValue: string | undefined): Promise<DesignSystemName> {
  return resolvePrompt(
    await select<DesignSystemName>({
...
async function findExistingInstallFiles(target: string, files: string[]): Promise<string[]> {
...
async function promptForConflictStrategy(existingFiles: string[]): Promise<"skip" | "overwrite"> {
  return resolvePrompt(
    await select<"skip" | "overwrite">({
...
async function promptForBootstrapPath(): Promise<BootstrapPath> {
  return resolvePrompt(
    await select<BootstrapPath>({
...
async function collectInstallOptions(
  resolvedTarget: string,
  providedPreset: PresetName | undefined,
  options: InitOptions,
): Promise<void> {
  if (!providedPreset) {
...
async function resolveConflictForInstall(
  resolvedTarget: string,
  options: InitOptions,
...
async function runSkillInstallInteractive(
  target: string | undefined,
  options: InitOptions,
  agentkitVersion: string,
): Promise<void> {
...
Examples:
  ...
  skill
    ...
    .option(
      "--design-system <name>",
      `store design system choice in config (${formatDesignSystemList()})`,
    )
    .action(async (target: string, options: InitOptions) => {
      await applyInitConfig(options, await loadConfigForTarget(target));
      const agentkitVersion = await readPackageVersion();
